...
public class Fmdl
{
    ...
    public void CreateAndSaveMesh(string path)
    {
        GameObject fmdlGameObject = new GameObject();
        fmdlGameObject.name = Path.GetFileNameWithoutExtension(path);
        ...
        for (int i = 0; i < objects.Length; i++)
        {
            ...
            Mesh mesh = new Mesh
            {
                vertices = unityMesh[i].vertices,
                uv = unityMesh[i].UVs,
                normals = unityMesh[i].normals,
                triangles = unityMesh[i].faces,
                boneWeights = unityMesh[i].boneWeights
            };
            ...
            subFmdlGameObjects[i].name = i.ToString();
            var meshPath = Path.GetDirectoryName(path) + "/" + Path.GetFileNameWithoutExtension(path) + i + ".asset";
            AssetDatabase.CreateAsset(mesh, meshPath);
        } //for
        

        PrefabUtility.CreatePrefab(prefabPath, fmdlGameObject, ReplacePrefabOptions.ConnectToPrefab);

        UnityEngine.Object.DestroyImmediate(fmdlGameObject);
        foreach (var subobj in subFmdlGameObjects)
        {
            UnityEngine.Object.DestroyImmediate(subobj);
        }
